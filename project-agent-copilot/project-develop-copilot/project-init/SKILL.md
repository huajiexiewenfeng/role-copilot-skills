...
- Dedicated Base Graph repositories are initialized by `project-base-init`, not ordinary `project-init`.
- If this skill is invoked directly for a Base Graph repo, explain the route mismatch and hand off to `project-base-init`.
- Do not create Base `manifest.json`, `project-catalog.md`, `overview.md`, `shared-edges.md`, `relation-policy.md`, or Base registry files from `project-init`.
